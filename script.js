...
async function onlineMove(row, col, letter, player) {
  ...
  try {
    ...
    if (!snap.exists()) throw new Error("Room not found. Create a new game link.");

    const remote = normalizeRemote(snap.data());
    if (remote.currentPlayer !== player) {
      messageEl.textContent = `It is ${playerLabel(remote.currentPlayer, remote.mode)}'s turn.`;
      return;
    }
    ...
    // setDoc with merge is more reliable on simple GitHub Pages apps than transactions.
    await setDoc(ref, next, { merge: true });
  } catch (err) {
    console.error("Firebase move save error:", err);
    const code = err && err.code ? err.code : "no-code";
    const msg = err && err.message ? err.message : String(err);
    messageEl.textContent = `Could not save move. Firebase says: ${code} — ${msg}`;
  }
}
...
async function createOnlineGame() {
  ...
  try {
    ...
    console.error(err);
    showSetupMessage(`Could not create online room. Firebase says: ${err?.code || "no-code"} — ${err?.message || err}`, true);
...
async function joinRoom(id) {
  ...
  try {
    ...
    showGame();
    state = freshState(10, "online");
    state.lastMessage = `Could not load room. Firebase says: ${err?.code || "no-code"} — ${err?.message || err}`;
    myOnlinePlayer = "";
    render();
  }
}
function listenToRoom(id) {
  ...
  unsubscribeRoom = onSnapshot(doc(db, "games", id), (snap) => {
    ...
    console.error(err);
    messageEl.textContent = `Firebase sync error: ${err?.code || "no-code"} — ${err?.message || err}`;
  });
}
